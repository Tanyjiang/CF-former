...
class Regression(nn.Module):
    def __init__(self, ):
        ...
        self.stage2_1 = nn.Sequential(
            nn.Conv2d(768, 128, 3, padding=1, dilation=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(768, 256, 3, padding=1, dilation=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True))

        self.conv1 = nn.Sequential(
            nn.Conv2d(256, 768, 3, padding=1, dilation=1),
            nn.BatchNorm2d(768),
            nn.ReLU(inplace=True))
        ...
        self.mask_cfg = [128]
        self.mask = make_layers(self.mask_cfg, in_channels=128, dilation=True)
        self.output_layer = nn.Conv2d(128, 1, kernel_size=1)
        self.att_encoder2 = nn.Sequential(nn.Conv2d(1, 512, kernel_size=3, padding=2, dilation=2),
                                          nn.ReLU(inplace=True),
    ...
    def forward(self, x1, x2, x3):
        ...
        x2_1 = self.v2(x2)  
        x3_1 = self.v3(x3)  
        intermediate = torch.cat((x1_1, x2_1, x3_1), dim=1)  


        middle1 = x1_1 - x2_1
        middle2 = x1_1 - x3_1
        middle3 = x2_1 - x3_1
        middle = torch.cat((middle1, middle2, middle3), dim=1)  
        middle = self.conv(middle)  
        middle = self.attn(middle) 
        middle = self.conv1(middle)  

        intermediate = intermediate * middle  
        y = self.stage2_1(intermediate) 

        x3_mask = self.mask(y)  
        x3_mask = self.output_layer(x3_mask)  
        x3_mask = self.soft_binary(x3_mask)  

        x3_new = torch.ones_like(x3_mask) 
        x3_new = x3_new - x3_mask
        x3_new = self.att_encoder2(x3_new) 
        y = torch.cat((y, x3_new), dim=1)  
        y = self.stage2_2(y) 
        y = self.res(y) 
        return y
...
def alt_gvt_large(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load('/home/shengqin/alt_gvt_large.pth')  # todo pass path as argument
