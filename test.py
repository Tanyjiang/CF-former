...
parser.add_argument('--crop_size', type=int, default=256,
                    help='the crop size of the test image for sha is 256,for shb is 512,for qnrf is 512,for nwpu is 256')
parser.add_argument('--model_path', default='pth/best_model_mae-50.82_epoch-514.pth', type=str,
                    help='saved model path')
parser.add_argument('--data_path', type=str, default='/home/dataset/part_A_final',
                    help='dataset path')
...
def test(args, isSave = True):
    ...
    for inputs, count, name in dataloader:
        with torch.no_grad():
            ...
            print("Img name: ", name, "Error: ", img_err, "GT count: ", count[0].item(), "Model out: ", torch.sum(outputs).item(), "error_rate:", img_error_rate)
            image_errs.append(img_err)




            result.append([name, count[0].item(), torch.sum(outputs).item(), img_err, img_error_rate])
